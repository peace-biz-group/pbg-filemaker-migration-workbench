...
export function writeSummaryMarkdown(outputDir: string, summary: ReportSummary, profile?: ProfileResult): void {
  ...
  if (profile) {
    ...
    for (const col of profile.columns) {
      const topVal = col.topValues[0]?.value ?? '—';
      const display = topVal.length > 30 ? topVal.slice(0, 27) + '...' : topVal;
      lines.push(
        `| ${col.name} | ${col.nonEmptyCount.toLocaleString()} | ${(col.missingRate * 100).toFixed(1)}% | ${col.uniqueCount.toLocaleString()} | ${display} |`,
      );
    }
    lines.push('');
    ...
    if (profile.anomalies.length > 0) {
      ...
      for (const a of sample) {
        const val = a.value.length > 30 ? a.value.slice(0, 27) + '...' : a.value;
        lines.push(`| ${a.row} | ${a.column} | ${val} | ${a.reason} |`);
      }
