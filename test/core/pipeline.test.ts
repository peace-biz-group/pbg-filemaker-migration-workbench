...
describe('Pipeline integration', () => {
  ...
  it('normalizes the sample file', async () => {
    const result = await normalizeFile(FIXTURE, config);
    expect(result.normalizedCount + result.quarantineCount).toBe(8);
  ...
  it('detects duplicates', async () => {
    const normResult = await normalizeFile(FIXTURE, config);
