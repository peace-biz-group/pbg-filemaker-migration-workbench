...
**大容量ファイルは必ず CSV で出力してください。**

| 用途 | 形式 |
|------|------|
| 列定義・件数の確認（小容量） | XLSX 可 |
| 本処理（10万行超 / 全件投入） | **CSV 必須**（UTF-8 または CP932） |

XLSX は SheetJS 経由でシート全体をメモリに展開するため、大容量ではメモリ不足になります。CSV はストリーム処理で逐次読み込むためファイルサイズに依存しません。

**FileMaker からの CSV エクスポート手順:**

1. FileMaker メニュー → ファイル → レコードのエクスポート
2. 「コンマ区切り (.csv)」を選択
3. 文字コード: **Unicode (UTF-8)** を推奨
4. UTF-8 で文字化けが出る場合は「日本語 (Shift-JIS)」(= CP932) を選択
